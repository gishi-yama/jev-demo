...
同じ自由記述文を、複数の視点で同時に構造化判定するデモを2種類収録。
...
```
server.js              APIキー中継サーバ(静的配信 + /api/analyze プロキシ)
...
Two demos that fire multiple structured questions over the same free-text input in parallel.
...
```
server.js                 Proxy server (static file serving + /api/analyze)
